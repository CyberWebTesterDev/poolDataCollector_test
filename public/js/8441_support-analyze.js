  ...
  constructor() {
    ...
    this.analyzeTasks = () => {
      ...
      if (taskTable) {
        ...
        tableId = "processData";
        rowsCount = document.querySelector("#processData > tbody").children.length;
        tdCount = document.querySelector("#processData > tbody").children[1].children.length;
      } else {
        console.log(`No active processes` + "\n");
        tableId = "tasksArchData";
        rowsCount = document.querySelector("#tasksArchData > tbody").children.length;
        tdCount = document.querySelector("#tasksArchData > tbody").children[1].children.length;
      }
      let tds = [];
      let i = taskTable ? 1 : 1;
      ...
      for (i; i < rowsCount; i++) {
        ...
        tds = document.querySelector(`#${tableId} > tbody`).children[i].children;

        for (let idx = 0; idx < tdCount; idx++) {
          console.log(`Analayzing td with index: ${idx} and value: ${tds[idx].innerText}` + "\n");
          ...
          if (tds[idx].innerText == "2") {
            if (rowsCount > 10 ? i <= 7 : i <= 5) {
              ...
              this.applicationState.tasks.errorActivityState.rowsIndexes.push(i);
            } else {
          ...
          if (tds[idx].innerText == "0") {
            if (tableId == "processData") {
              ...
              this.applicationState.tasks.currentActivityState.isIntermediate
                ? (this.applicationState.tasks.currentActivityState.stateDescription = "Зависание")
              ...
              this.applicationState.tasks.currentActivityState.nullStateIndexes.push(i);
            } else {
              ...
              this.applicationState.tasks.currentActivityState.activityType = tds[3].innerText;
              this.applicationState.tasks.currentActivityState.nullStateCounter++;
              this.applicationState.tasks.currentActivityState.nullStateIndexes.push(i);
            }
          }
    ...
    this.checkIntegrationLogs = () => {
      ...
      const rowsLength = document.querySelector("#integrationLogData > tbody").children.length;
      const tdsLength = document.querySelector("#integrationLogData > tbody").children[1].children
        .length;
      let tds = [];
      ...
      for (let i = 1; i < rowsLength; i++) {
        ...
        tds = document.querySelector("#integrationLogData > tbody").children[i].children;

        for (let idx = 0; idx < tdsLength; idx++) {
          console.log(`Analayzing integration table td with index: ${idx}` + "\n");

          if (tds[idx].innerText && idx == 6) {
            console.log(`Integration error text was detected on row index: ${i}` + "\n");
